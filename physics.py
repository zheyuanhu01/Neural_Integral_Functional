...
def poisson(x, w, B=10000, N=20, ve=False):
    a = (np.random.rand(B, N) - 0.5) * 2
    def poly(x):
        return (a @ (np.reshape(x, (-1, 1)) ** np.arange(N)).T)
    def nabla_poly(x):
...
if __name__ == "__main__":
    ...
    if args.dataset == "ke":
        ...
        x, w = np.polynomial.legendre.leggauss(args.N_grid)
        train_n, train_nabla_n, train_nabla2_n, train_m, train_y, train_dy, test_n, test_nabla_n, test_nabla2_n, test_m, test_y, test_dy = poisson(x, w, B=args.N_data, ve=args.ve)
    gridx = x
    @hk.transform
    ...
    if args.dataset == "poisson":
        ...
        np.savetxt("x.txt", np.asarray(x))
        np.savetxt("y_true.txt", np.asarray(y_true))
        np.savetxt("y_pred.txt", np.asarray(y_pred))
        pass
    
"""
[-1.0030185  -0.0022855   1.0070635   0.01311341 -0.0262247  -0.03772518
  0.03717938  0.05923396 -0.00894638 -0.05171974  0.06299371 -0.00890488
 -0.19333772  0.06099257  0.08930872  0.02142952  0.1167045  -0.11829557
 -0.08310117  0.06397512]
"""